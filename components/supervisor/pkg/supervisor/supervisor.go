...
func runIDEReadinessProbe(cfg *Config, ideConfig *IDEConfig, ide IDEKind) (desktopIDEStatus *DesktopIDEStatus) {
	...
	case ReadinessHTTPProbe:
		...
		for range time.Tick(250 * time.Millisecond) {
			desktopIDEStatus, err = ideStatusRequest(url)
			if err != nil {
...
func ideStatusRequest(url string) (*DesktopIDEStatus, error) {
	...
	var desktopIDEStatus DesktopIDEStatus
	err = json.Unmarshal(bodyBytes, &desktopIDEStatus)
	if err != nil {
		return nil, xerrors.Errorf("unexpected error parsing IDE status JSON: %w", err)
	}

	return &desktopIDEStatus, nil
}
